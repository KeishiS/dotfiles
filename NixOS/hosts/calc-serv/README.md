...
`hosts/lenovo/nextcloud.nix` では `user_oidc` appを同梱し、`nextcloud-kanidm-oidc.service` でproviderを設定する。
...
Lenovo側には次のsecretを置く。

```text
hosts/lenovo/secrets/nextcloud-smtp-password.enc
  Resend API key

hosts/lenovo/secrets/nextcloud-oidc-client-secret.enc
...
```bash
git add \
  NixOS/hosts/lenovo/secrets/nextcloud-smtp-password.enc \
  NixOS/hosts/lenovo/secrets/nextcloud-oidc-client-secret.enc
```

Nextcloud側では `user_oidc` appを有効化し、providerを作成する。
この設定は `nextcloud-kanidm-oidc.service` が自動実行する。
手動で確認・再設定する場合のコマンドは次の通り。
